...
# --- 2. 智谱 AI 战略模块 (终极加固：100秒超时 + 5次暴力重试) ---
def get_ai_advice(vix, total_amt, results):
    ...
    for attempt in range(5):
        try:
            ...
            # timeout=(连接超时, 读取超时) -> 15秒建立连接，100秒等待回信
...
def send_report(title, total_rmb, results, vix, ai_advice):
    ...
    <div style="font-family:'Roboto',Arial,sans-serif; max-width:650px; margin:auto; border:1px solid #dadce0; border-radius:8px; overflow:hidden; background:#fff;">
        <div style="padding:20px 24px; border-bottom:1px solid #dadce0;">
            ...
            <span style="background:#e8f0fe; color:#1a73e8; padding:2px 8px; border-radius:12px; font-size:10px; margin-left:10px; font-weight:600;">PRO SENTINEL 5.5</span>
        </div>
        <div style="padding:24px;">
            ...
            <div style="background:#f8f9fa; border-radius:8px; padding:24px; border:1px solid #eee;">
                <div style="font-size:15px; color:#202124; font-weight:500; margin-bottom:12px;">战略执行决策建议 (智谱 AI)</div>
        ...
        <div style="background:#f1f3f4; padding:16px 24px; text-align:center; color:#70757a; font-size:11px;">
            Sentinel Pro 5.5 (Region: East US) · {datetime.now().strftime('%Y-%m-%d %H:%M')}
        </div>
    </div>"""
...
def main():
    ...
    for name, info in assets.items():
        try:
            ...
            total_all += rmb_amt
        exceptException as e: print(f"Skip: {name} - {e}")
    ...
    if results:
        ...
        send_report(f"Strategic Portfolio: {datetime.now().strftime('%m/%d')} 决策简报", total_all, results, round(ctx['VIX'], 1), ai_res)
